 ...
 * modified from https://github.com/freedesktop-unofficial-mirror/evtest/
 * commit b8343ec1124da18bdabcc04809a8731b9e39295d 
 * to accomodate same key-hook functions ...
 */
 ...
 * @return 0 on success or 1 otherwise.
 */
static int print_events(int fd, const char *device)
{
    ...
    char key_map[KEY_MAX + 1];    //  Create a byte array the size of the number of keys
    char buf[160];
    char shName[60];

	static const char stateFolder[] = "/dev/input/evkey";
    static const char shortkFolder[] = "/dev/input/evkey-shortcuts";
	...
	//int ifd = open("/dev/input/uinput",O_WRONLY);
	//ioctl(fd, EVIOCGRAB, (void*)0);
	//ioctl(fd, EVIOCGRAB, (void*)1);
	//char* filename = strdup(device);
	//int ifd = open(filename,O_WRONLY);
	...
	while (!stop) {
		...
		for (i = 0; i < rd / sizeof(struct input_event); i++) {
			...
			if (type == EV_SYN) {
				...
				if (wasChange){
					...
					if (globbuf.gl_pathc > 0)
    					{
						printf("shortkFolder/Count:%lu\n",globbuf.gl_pathc);
        				//int fd = open(globbuf.gl_pathv[0], O_RDONLY);
						sprintf(buf, "%s",globbuf.gl_pathv[0]);
    					}
					else {
						printf("shortkFolder/Count:0\n");
						sprintf(buf, "%s/%s",shortkFolder, shName);
					}
			...
			if (!handled){
				...
				evi.code = ev[i].code;
				evi.type = ev[i].type;
				evi.value = ev[i].value;
				ioctl(fd, EVIOCGRAB, (void*)0);
				ignore=true;
				if (write(ifd, &evi, sizeof(evi)) != sizeof(evi)) {
